...
public class ManejadorXML extends DefaultHandler implements ParserCatalogo {
	...
	/*
	 * uri es el Namespace -->"http://www.piat.upm.es/catalogo"
	 * localName --> catalog
	 * qName es el nombre completo, osea el xmlns+nombreLocal
	 * attributes son los atributos del elemento
	 */
	...
	public void startElement(String uri, String localName, String qName, Attributes attributes) throws SAXException {
		...
		switch(qName) {
			...
			break;
			case "concept":
				//Detectar cuando llega un evento startElement del elemento <concept> para
				//guardar temporalmente el valor del atr id y anotar que se ha encontrado un <concept>
				...
				/*Recoger los dataset 1º:Cuando llegue un evento starElement <datasets>, llegue el evento startElement <dataset>, llegue el evento startElement <concepts> y
				 * llegue el evento startElement <concept> donde su atributo id coincida con alguno de las url almacenadas en el ArrayList lConcepts, 
					...
					for (String aux : lConcepts)
						if(aux.equals(idConcept)) {
							if(hData.containsKey("title") && hData.containsKey("description") && hData.containsKey("theme")
							   && !hDatasets.containsKey(idData) && idConcept.contains(lConcepts.get(0))) { //idConcept (parte ccomun) y lConcepts(parte entera del id)
